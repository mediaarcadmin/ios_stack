...
//	NSLog(@"BlioFlowPaginateOperation start entered: %@",self); 
    ...
    // Create a EucBook for the paginator.
    {
		...
		// AC TESTING:  check if license bind is successful for book;
		// if not, abort
		// Will this work?? 
		// if ( [[book xpsProvider] decryptionAvailable] ) {
...
//	NSLog(@"Book %@ pagination progress: %f",self.bookTitle,self.percentageComplete);
}
-(void)cancel {
	[super cancel];
