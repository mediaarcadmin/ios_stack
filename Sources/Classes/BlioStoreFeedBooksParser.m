...
@interface BlioStoreFeedBooksParser()
...
@property (nonatomic, retain) NSString *currentReleasedYear;
...
@synthesize delegate, parsedCategories, parsedEntities;
@synthesize currentString, currentCategory, currentEntity, currentTitle, currentAuthorName, currentSummary, currentEPubUrl, currentPdfUrl, currentCoverUrl, currentThumbUrl, currentReleasedYear, currentPublishedDate, currentBaseURL, atomDateFormatter, xmlData, feedConnection, downloadAndParsePool;
...
- (void)finishedCurrentEntity {
    ...
    [self.currentEntity setReleasedYear:self.currentReleasedYear];
    ...
    if ([elementName isEqualToString:kBlioStoreFeedBooksParserName_Feed]) {
        ...
        if ([type isEqualToString:@"application/atom+xml"]) {
            if (nil != href) {
                ...
                [self.currentCategory setUrl:[absoluteURL absoluteString]];
            }
        } else if ([type isEqualToString:@"application/atom+xml;type=entry"]) {
            if (nil != href) {
                self.currentEntity = [[[BlioStoreParsedEntity alloc] init] autorelease];
                NSURL *absoluteURL = [NSURL URLWithString:href relativeToURL:self.currentBaseURL];
                [self.currentEntity setUrl:[absoluteURL absoluteString]];
            }
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
    if ([elementName isEqualToString:kBlioStoreFeedBooksParserName_Feed]) {
        ...
        self.currentReleasedYear = [NSString stringWithString:self.currentString];
...
@synthesize title, author, url, summary, ePubUrl, pdfUrl, coverUrl, thumbUrl, releasedYear, publishedDate;
