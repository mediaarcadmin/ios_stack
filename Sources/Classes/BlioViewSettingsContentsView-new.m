...
- (id)initWithDelegate:(id)newDelegate {
	...
	if ((self = [super initWithFrame:CGRectZero])) {
        ...
        if(voiceOverIsRelevant) {
            aTabZoomsToBlockLabel.text = NSLocalizedString(@"Reading",@"Settings Label for \"Tap Advance\" Segmented Control when VoiceOver is running (for VoiceOver users, the setting is not related to tapping, but to how VoiceOver selects text).");
        } else {
        ...
        if(voiceOverIsRelevant) 
            tapAdvanceTitles = [NSArray arrayWithObjects:
                                NSLocalizedString(@"Continuous",@"\"Continuous\" segment label (for \"VoiceOver\" SegmentedControl)"),
                                NSLocalizedString(@"By Block",@"\"By Block\" segment label (for \"VoiceOver\" SegmentedControl)"),
                                nil];
        else
            tapAdvanceTitles = [NSArray arrayWithObjects:
        ...
        if(voiceOverIsRelevant) {
            [aTapZoomsToBlockSegment setAccessibilityHint:NSLocalizedString(@"In this mode, swipe down with two fingers to begin reading from the current position. Pages will turn automatically. Tap the page to stop reading, or to read individual lines.", @"Accessibility hint for View Settings \"By Page\" button (for \"Tap Advance\" SegmentedControl)") forSegmentIndex:0];
            [aTapZoomsToBlockSegment setAccessibilityHint:NSLocalizedString(@"In this mode, tap the page to read blocks of text.  Swipe down with two fingers to read the current page.  Pages will not turn automatically.", @"Accessibility hint for View Settings  \"By Block\" button (for \"Tap Advance\" SegmentedControl)") forSegmentIndex:1];
        }
