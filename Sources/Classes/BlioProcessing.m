...
@synthesize bookID, songID, sourceID, sourceSpecificID, forceReprocess, cacheDirectory,tempDirectory;
...
- (void)flushBookCache {
    ...
    pthread_mutex_lock(&sBookMutationMutex);
    {
		...
		if (!self.bookID)
            NSLog(@"WARNING: self.bookID for %@ is nil, flushBookCache will fail!",[self description]);
...
- (void)setSongValue:(id)value forKey:(NSString *)key {
    ...
    pthread_mutex_lock(&sBookMutationMutex);
    {
        ...
        BlioBookManager *songManager = [BlioBookManager sharedBookManager];
        BlioSong *song = [songManager songWithID:self.songID];
        if (nil == song) {
            NSLog(@"Failed to retrieve song in BlioProcessing setSongValue:forKey:");
        } else {
            [song setValue:value forKey:key];
        }
        NSError *anError;
        if (![songManager save:&anError]) {
            NSLog(@"[BlioProcessingOperation setSongValue:%@ forKey:%@] Save failed with error: %@, %@", value, key, anError, [anError userInfo]);
        }
        --mutationCount;
    }
    pthread_mutex_unlock(&sBookMutationMutex);
...
- (id)getSongValueForKey:(NSString *)key {
    ...
    if (nil == song) {
        NSLog(@"Failed to retrieve song in BlioProcessing getSongValueForKey:");
        return nil;
    } else {
