...
- (void)_paginationThread
{
    ...
    EucBookTextView *bookTextViews[sDesiredPointSizesCount];
    memset(bookTextViews, 0, sizeof(bookTextViews));
    ...
    for(NSUInteger i = 0; i < sDesiredPointSizesCount; ++i) {
        bookTextViews[i] = [[EucBookTextView alloc] initWithFrame:[EucPageView bookTextViewFrameForPointSize:sDesiredPointSizes[i]] pointSize:sDesiredPointSizes[i]];
        if(!bookTextViews[i]) {
...
//         usleep(100000);
        ...
        nextPoint = [bookReader.book.pageLayoutControllerClass layoutPageFromBookReader:bookReader
                                                            ...
                                                            intoView:bookTextViews[currentPointIndex]];
        ...
        [bookTextViews[currentPointIndex] clear];
...
- (void)_paginateBook:(id<EucBook>)book inBackground:(BOOL)inBackground forForFontFamily:(NSString *)fontFamilyName
{
...
{
    [self _paginateBook:book inBackground:YES forForFontFamily:fontFamilyName];
}

- (void)testPaginateBook:(id<EucBook>)book forForFontFamily:(NSString *)fontFamilyName
{
    [self _paginateBook:book inBackground:NO forForFontFamily:fontFamilyName];
}
